...
class Gethomeworksolutaionapi {
  Dio dio = Dio();
  Gethomeworksolutaion({id}) async {
    final controller = Get.find<Homeworksolutioancontroller>();
    try {
      controller.SetIsLoading(true);
      String myurl = "$hostPort$getstudentSolution";
      Get.find<Homeworksolutioancontroller>().setisCurriculmLoading(true);
      Get.find<Homeworksolutioancontroller>().setisCurriculmDialogLoading(true);
      var response = await dio.post(data: {
        "homeworkId": '$id',
      }, myurl, options: getDioOptions());
      if (response.statusCode == 200) {
        
        HomeworkSolutionModel homeworkLMSModel =
            HomeworkSolutionModel.fromJson(response.data);
