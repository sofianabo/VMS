...
import 'package:vms_school/Link/Model/LMS_Model/StudentModel/studentHomeworkModel.dart';

class Studenthomeworkapi {
  Dio dio = Dio();
  Studenthomework({id}) async {
    final controller = Get.find<Studenthomeworkcontroller>();
    try {
      controller.SetIsLoading(true);
      String myurl = "$hostPort$getHomework";
      Get.find<Studenthomeworkcontroller>().setisCurriculmLoading(true);
      Get.find<Studenthomeworkcontroller>().setisCurriculmDialogLoading(true);
      ...
      if (response.statusCode == 200) {
        ...
        studentHomeworkModel homeworkLMSModel =
            studentHomeworkModel.fromJson(response.data);
