...
class Addlinklmsapi {
  ...
  Addlinklmsapi(this.context);
  Dio dio = Dio();
  Curriculumn_LMS_Controller controller =
      Get.find<Curriculumn_LMS_Controller>();
  LinkslmsController linkcontrol = Get.find<LinkslmsController>();

  Addlinklms({int? curriculumId, name, link, required bool hidden}) async {
    CancelToken cancelToken = CancelToken();
    Loading_Dialog(cancelToken: cancelToken);
    try {
      int? currid = controller.curriculum![curriculumId!].id;

      String myurl = "$hostPort$addLmsCurriculmUrl";
      ...
      if (response.statusCode == 200) {
        gets.Get.back();
        gets.Get.back();
        LinksLMSModel lms_files_model = LinksLMSModel.fromJson(response.data);
        linkcontrol.SetLinks(lms_files_model);
      } else {
