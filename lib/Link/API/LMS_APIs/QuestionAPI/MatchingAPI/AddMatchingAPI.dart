...
class Add_Matching_api {
  Dio dio = Dio();

  Add_Matching({
    required String Question,
    required List<String> ques,
    required List<String> ans, // تعديل لدعم أكثر من إجابة صحيحة
  }) async {
    CancelToken cancelToken = CancelToken();
    Loading_Dialog(cancelToken: cancelToken);
    try {
      var controller = Get.find<Matching_Question_Controller>();
      ...
      if (response.statusCode == 200) {
        gets.Get.back();
        gets.Get.back();
        MatchingQuestionModel chooes_the_correct_model =
            MatchingQuestionModel.fromJson(response.data);
        controller.addQuestion(chooes_the_correct_model.question![0]);
      } else {
