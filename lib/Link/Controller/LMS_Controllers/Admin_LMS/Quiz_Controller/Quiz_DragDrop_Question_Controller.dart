...
class Quiz_DragDrop_Question_Items {
  String? text;
  String? imagePath;
  Uint8List? imageBytes; // إضافة هذا الحقل لتخزين بايتات الصورة
  bool showTextField;

  Quiz_DragDrop_Question_Items({
    this.text,
    this.imagePath,
    this.imageBytes,
...
class Quiz_DragDrop_Question_Controller extends GetxController {
  String? filterName = '';
  List<Question>? questions;
  List<Question>? filterdQuestions;

  // قوائم الخيارات للقسمين
  final RxList<Quiz_DragDrop_Question_Items> firstSectionOptions =
      <Quiz_DragDrop_Question_Items>[].obs;
  final RxList<Quiz_DragDrop_Question_Items> secondSectionOptions =
      <Quiz_DragDrop_Question_Items>[].obs;
  ...
  void addOption() {
    firstSectionOptions.add(Quiz_DragDrop_Question_Items());
    secondSectionOptions.add(Quiz_DragDrop_Question_Items());
    update();
  }
  ...
  void updateFirstSectionText(int index, String text) {
    if (index < firstSectionOptions.length) {
      firstSectionOptions[index] = Quiz_DragDrop_Question_Items(
        text: text,
        imagePath: null,
  ...
  void toggleTextField(int index, bool isFirstSection) {
    if (isFirstSection) {
      ...
      secondSectionOptions[index] = Quiz_DragDrop_Question_Items(
