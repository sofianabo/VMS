              ...
              child: DropdownButton<String>(
                    ...
                    switch (type) {
                      case 'grade':
                        IncreaseAttendanceAPI(context).GetIncreaseAttendance(
                          isserch: true,
                          gradeid: Get.find<Dropdowngradecontroller>().gradess!.grades!
                        .firstWhere((grad) => grad.enName == newValue||grad.name == newValue).id.toString(),
                        );
                        break;
                        case 'class':
                        IncreaseAttendanceAPI(context).GetIncreaseAttendance(
                          isserch: true,
                          classId: Get.find<Divisions_Controller>().Classmodel!.classes!
                              .firstWhere((cls) => cls.enName == newValue||cls.name == newValue).id.toString(),
                        );
                        break;
                      case 'division':
                        IncreaseAttendanceAPI(context).GetIncreaseAttendance(
                          isserch: true,
                          divisionId: Get.find<Dropdowndivisioncontroller>().allDivision
                              .firstWhere((cls) => cls.enName == newValue||cls.name == newValue).id.toString() ,
                        );
                        break;
                    }
                ...
                icon:  selectedValue.isNotEmpty && selectedValue != title
                  ...
                  onTap: () {
                    ...
                    if(type == "division"){
                      print(Get.find<Divisions_Controller>().Classmodel!.classes!
                          .firstWhere((cls) => cls.enName == cont.classIndex || cls.name == cont.classIndex).id.toString());
                      IncreaseAttendanceAPI(context).GetIncreaseAttendance(
                          isserch: true,
                          classId: Get.find<Divisions_Controller>().Classmodel!.classes!
                              .firstWhere((cls) => cls.enName == cont.classIndex || cls.name == cont.classIndex).id.toString());
